...
\vspace{-1ex}
\subsubsection*{WP 1: Input scenarios}
%

\label{sec:atmscenes}
The scenarios will be extracted from CARRA reanalyses
(Sec.~\ref{sec:harmonie}). CARRA does not cover the full target area, but this
...
made (Sec.~\ref{sec:sar}). The fine spatial resolution of SAR will
...
(Sec.~\ref{sec:preliminary_results}) to include regions north of 65$^\circ$,
...
This WP will run concurrently with WP 3 and WP 6. We will set-up test retrievals as soon as few hundred thousand simulations are available, so that an early feedback on any database issues can be made. Besides, during the development phase, a basic evaluation (Sec.~\ref{sec:evaluation}) of the retrievals with existing retrieval products will be made to identify the areas requiring improvement. Repetition of the retrievals will be made accordingly.
